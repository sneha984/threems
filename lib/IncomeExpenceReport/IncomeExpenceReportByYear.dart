...
class IncomeExpenseYearReport extends StatefulWidget {
  ...
  @override
  State<IncomeExpenseYearReport> createState() => _IncomeExpenseYearReportState();
}
      ...
      if(event.docs.isNotEmpty) {
        ...
        for (DocumentSnapshot data in event.docs) {
          ...
          if(expenseCategory.contains(data['categoryName'])){
            ...
            expenseList.insert(expenseCategory.indexOf(data['categoryName']), item);
          }else{
  ...
  getYearMonthExpenses(){
    ...
    snapshots().listen((event) {
      ...
      if(event.docs.isNotEmpty) {
        ...
        for (DocumentSnapshot data in event.docs) {
          if(monthsExpenseList.contains(data['date'].toDate().toString().substring(0,7))){
            Map<String,dynamic> item=monthlyExpenseReports[monthsExpenseList.indexOf(data['date'].toDate().toString().substring(0,7))];
            double amount=item['amount'];
            amount+=data['amount'];
            item['amount']=amount;
            ...
            monthlyExpenseReports.removeAt(monthsExpenseList.indexOf(data['date'].toDate().toString().substring(0,7)));
            monthlyExpenseReports.insert(monthsExpenseList.indexOf(data['date'].toDate().toString().substring(0,7)), item);

          }else{
            // expenseCategory.add(data['date'].toDate().toString().substring(5,7));
            // expenseList.add(data);
            monthsExpenseList.add(data['date'].toDate().toString().substring(0,7));
            ...
            Column(
              children: [
                ...
                Container(
                  ...
                  Padding(
                    padding:  EdgeInsets.only(top: scrWidth*0.2,left:scrWidth*0.04,right: scrWidth*0.01 ),
                    child: Container(
                      width: MediaQuery.of(context).size.width,
                              ...
                              return DataRow(
                                cells: [
                                  ...
                                  DataCell(
                                      Padding(
                                        padding: const EdgeInsets.only(left: 8.0),
                                        child:IncomeExpenseList[index]['income']==false?
                                        Text('-'+IncomeExpenseList[index]['amount'].toString(),
                                          style: TextStyle(fontSize: 13,
                                              fontFamily: 'Urbanist',
                                              color: Colors.red
                  ...
                  Padding(
                    padding:  EdgeInsets.only(top: scrWidth*0.2,left:scrWidth*0.04, ),
                    child: Container(
                      width: MediaQuery.of(context).size.width,
                          ...
                          columns: [
                            ...
                            DataColumn(
                              label: Padding(
                                ...
                                child: Text("INCOME",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 13)),
                              ),
                            ),
                          ],
                          rows: List.generate(
